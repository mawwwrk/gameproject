export function hitTestCircle(c1, c2, global = false) {
  let vx, vy, magnitude, combinedRadii, hit; //Calculate the vector between the circles' center points
  ...
  magnitude = Math.sqrt(vx * vx + vy * vy); //Add together the circles' combined radii

  combinedRadii = c1.radius + c2.radius; //Set `hit` to `true` if the distance between the circles is //less than their ` combinedRadii `
...
export function hitTestRectangle(r1, r2, global = false) {
  let hit, combinedHalfWidths, combinedHalfHeights, vx, vy; //A variable to determine whether there's a collision
...
export function rectangleCollision(r1, r2, bounce = false, global = true) {
  let collision,
    ...
    overlapY,
    vx,
    vy; //Calculate the distance vector
  ...
  if (Math.abs(vx) < combinedHalfWidths) {
    ...
    if (Math.abs(vy) < combinedHalfHeights) {
      ...
      overlapY = combinedHalfHeights - Math.abs(vy); //The collision has occurred on the axis with the //*smallest* amount of overlap. Let's figure out which //axis that is
...
          */
    ...
    //No collision
  } //Return the collision string. It will be either "top", "right", //"bottom", or "left" depending on which side of r1 is touching r2.
...
export function circleCollision(c1, c2, bounce = false, global = false) {
  ...
  if (magnitude < combinedRadii) {
    ...
    overlap = combinedRadii - magnitude; //Add some "quantum padding"

    let quantumPadding = 0.3;
    overlap += quantumPadding; //Normalize the vector //These numbers tell us the direction of the collision

    dx = vx / magnitude;
    dy = vy / magnitude; //Move circle 1 out of the collision by multiplying //the overlap with the normalized vector and subtract it from //circle 1's position
    ...
    if (bounce) {
      ...
      s.x = vy;
      s.y = -vx; //Bounce c1 off the surface //(I’ll explain this ahead)
...
export function movingCircleCollision(c1, c2, global = false) {
  let combinedRadii,
    overlap,
    xSide,
    ySide, //`s` refers to the distance vector between the circles
    s = {},
    p1A = {},
    p1B = {},
    p2A = {},
  ...
  if (s.magnitude < combinedRadii) {
    ...
    overlap += 0.3; //Normalize the vector. //These numbers tell us the direction of the collision

    s.dx = s.vx / s.magnitude;
    s.dy = s.vy / s.magnitude; //Find the collision vector. //Divide it in half to share between the circles, and make it absolute

    s.vxHalf = Math.abs((s.dx * overlap) / 2);
    s.vyHalf = Math.abs((s.dy * overlap) / 2); //Find the side on which the collision is occurring

    c1.x > c2.x ? (xSide = 1) : (xSide = -1);
    c1.y > c2.y ? (ySide = 1) : (ySide = -1); //Move c1 out of the collision by multiplying //the overlap with the normalized vector and adding it to //the circles’ positions

    c1.x = c1.x + s.vxHalf * xSide;
    c1.y = c1.y + s.vyHalf * ySide; //Move c2 out of the collision

    c2.x = c2.x + s.vxHalf * -xSide;
    c2.y = c2.y + s.vyHalf * -ySide; //Now that the circles have been separated, you can bounce //them apart. The code below does this in 4 major, numbered steps: //Find the surface vector's left normal

    //1. Calculate the collision surface's properties

    s.lx = s.vy;
    s.ly = -s.vx; //2. Bounce c1 off the surface (s) //Find the dot product between c1 and the surface

    let dp1 = c1.vx * s.dx + c1.vy * s.dy; //Project c1's velocity onto the collision surface

    p1A.x = dp1 * s.dx;
    p1A.y = dp1 * s.dy; //Find the dot product of c1 and the surface's left normal (s.lx and s.ly)

    let dp2 = c1.vx * (s.lx / s.magnitude) + c1.vy * (s.ly / s.magnitude); //Project c1's velocity onto the surface's left normal

    p1B.x = dp2 * (s.lx / s.magnitude);
    p1B.y = dp2 * (s.ly / s.magnitude); //3. Bounce c2 off the surface (s) //Find the dot product between c2 and the surface

    let dp3 = c2.vx * s.dx + c2.vy * s.dy; //Project c2's velocity onto the collision surface

    p2A.x = dp3 * s.dx;
    p2A.y = dp3 * s.dy; //Find the dot product of c2 and the surface's left normal (s.lx and s.ly)

    let dp4 = c2.vx * (s.lx / s.magnitude) + c2.vy * (s.ly / s.magnitude); //Project c2's velocity onto the surface's left normal

    p2B.x = dp4 * (s.lx / s.magnitude);
    p2B.y = dp4 * (s.ly / s.magnitude); //4. Calculate the bounce vectors //Bounce c1 using p1B and p2A

    c1.bounce = {};
    c1.bounce.x = p1B.x + p2A.x;
    c1.bounce.y = p1B.y + p2A.y; //Bounce c2 using p1A and p2B

    c2.bounce = {};
    c2.bounce.x = p1A.x + p2B.x;
    c2.bounce.y = p1A.y + p2B.y; //Add the bounce vector to the circles' velocity //and add mass if the circle has a mass property
...
export function hitTestPoint(point, sprite) {
  let shape, left, right, top, bottom, vx, vy, magnitude, hit; //Find out if the sprite is rectangular or circular depending //on whether it has a `radius` property
  ...
  if (shape === "circle") {
    ...
    (vx = point.x - sprite.centerX),
      ...
      (magnitude = Math.sqrt(vx * vx + vy * vy)); //The point is intersecting the circle if the magnitude //(distance) is less than the circle's radius
...
export function hit(
  ...
  } //Return the result of the collision. //It will be `undefined` if there's no collision and `true` if //there is a collision. `rectangleCollision` sets `collision` to //"top", "bottom", "left" or "right" depending on which side the //collision is occurring on
  ...
  function findCollisionType(a, b) {
    ...
    } //They're not both sprites, so what are they? //Is `a` not a sprite and does it have x and y properties?
    else if (bIsASprite && !(a.x === undefined) && !(a.y === undefined)) {
      //Yes, so this is a point vs. sprite collision test
      return hitTestPoint(a, b);
    } else {
      //The user is trying to test some incompatible objects
      throw new Error(`I'm sorry, ${a} and ${b} cannot be used together in a collision test.'
      `);
    }
  }
